...
# https://github.com/pedrohcgs/claude-code-my-workflow/blob/main/.claude/hooks/pre-compact.py
"""
...
def extract_recent_decisions(project_dir: str, limit: int = 3) -> list[str]:
    ...
    log_files = sorted(
        logs_dir.glob("*.md"), key=lambda f: f.stat().st_mtime, reverse=True
    )
    if not log_files:
        return []
...
def should_block_draft(plan_info: dict | None) -> tuple[bool, str]:
    ...
    try:
        sentinel_file.write_text(
            json.dumps(
                {"last_blocked_plan": plan_path, "when": datetime.now().isoformat()}
            )
        )
    except OSError as e:
        print(
            f"Warning: could not persist block sentinel; not blocking: {e}",
            file=sys.stderr,
        )
        return False, ""
...
def append_to_session_log(project_dir: str, trigger: str) -> None:
    ...
    try:
        with open(log_files[0], "a") as f:
            f.write(f"\n\n---\n")
            f.write(
                f"**Context compaction ({trigger}) at {datetime.now().strftime('%H:%M')}**\n"
            )
