...
# Both layers of the r-stack are drawn from one base, so the panel, the axes,
# and the point positions land on the same pixels and the overlay registers on
# top of the plain ranking.
board_base <- ggplot(board, aes(x = score, y = system)) +
...
board_base + geom_point(size = 4, colour = col_muted)
```

::: {.fragment fragment-index="2"}
```{r}
#| label: board-intervals
#| fig-width: 8
#| fig-height: 4.0
board_base +
  geom_vline(xintercept = max(board$lo), linetype = "dashed",
             colour = col_flag, linewidth = 0.6) +
  geom_errorbarh(aes(xmin = lo, xmax = hi, colour = tied),
                 height = 0.28, linewidth = 1.1) +
  geom_point(aes(colour = tied), size = 4)
```
:::
:::
...
runs [3.5 standard errors]{.fragment .highlight-accent} wide.
...
## Before next class {#next-steps .closing-slide transition="fade"}
