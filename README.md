...
Four pages worth reading before adapting any of this: Anthropic's [prompting best practices](https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/claude-prompting-best-practices) and its companion for [prompting Claude Fable 5.1](https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/prompting-claude-fable-5-1), the [Claude Code best practices](https://code.claude.com/docs/en/best-practices), and the post on [getting the most out of Claude Code sessions](https://claude.com/blog/maximizing-the-value-of-your-claude-code-sessions). The configuration in this repository is checked against them.
...
|---|---|---|
| `causal-design` | Triages a causal question to the identification strategy the data can actually support, then hands off to the skill that owns it. Carries the selection-on-observables branch itself: overlap, doubly robust estimation, double machine learning, causal forests, sensitivity analysis. | [Imbens (2024)](https://doi.org/10.1146/annurev-statistics-033121-114601); [Li et al. (2024)](https://www.ama.org/marketing-news/causal-inference-with-quasi-experimental-data/); [Abadie et al. (2023)](https://arxiv.org/abs/1710.02926); [Cunningham, Mixtape ch. 8](https://mixtape.scunning.com/08-panel_data) |
| `did` | Runs a difference-in-differences analysis with the heterogeneity-robust estimators, event-study diagnostics, honest bounds on pre-trend violations, and the estimand-and-scale choice for heavy-tailed, binary, or count outcomes (PPML, nonlinear DiD, repeated cross sections). | [Roth et al. (2023)](https://arxiv.org/abs/2201.01194); [Baker et al. (2026)](https://arxiv.org/abs/2503.13323); [Arkhangelsky and Imbens (2024)](https://doi.org/10.1093/ectj/utae014); [Abadie et al. (2025)](https://www.nber.org/papers/w34550); [MacKinnon et al. (2023)](https://doi.org/10.1016/j.jeconom.2022.04.001); [Winkler et al. (2026)](https://doi.org/10.1007/s11129-026-09312-2); [Wooldridge (2026)](https://doi.org/10.1257/pandp.20261111); [Cunningham, Mixtape ch. 9](https://mixtape.scunning.com/08a-difference_in_differences) and [ch. 10](https://mixtape.scunning.com/08b-complex_diff_in_diff) |
| `synthetic-control` | Builds and validates synthetic control comparisons, including synthetic difference-in-differences, the augmented and penalized variants, and factor-model alternatives. | [Abadie (2021)](https://doi.org/10.1257/jel.20191450); [Arkhangelsky and Imbens (2024)](https://doi.org/10.1093/ectj/utae014); [Cunningham, Mixtape ch. 11](https://mixtape.scunning.com/09-synthetic_control) |
| `rdd` | Runs a regression discontinuity in both the continuity and local-randomization frameworks, with the design gate and the full falsification battery. | [Cattaneo and Titiunik (2022)](https://doi.org/10.1146/annurev-economics-051520-021409); [Cattaneo et al. (2023)](https://arxiv.org/abs/2302.07413); [Cunningham, Mixtape ch. 6](https://mixtape.scunning.com/06-regression_discontinuity) |
...
- MCP integrations worth setting up:
  ...
  - Claude in Chrome: Claude Code's built-in browser integration (`claude --chrome`, [docs](https://code.claude.com/docs/en/chrome)), which drives your own Chrome with its logins in it. Concrete uses: booking flights and comparing options across sites, filling out forms (registrations, IRB portals, survey platforms), reading pages that sit behind a login or only render with JavaScript, and scraping structured data. Each session gets its own tab group, so parallel sessions do not queue on one browser. It is a visible window at your screen's size with no headless mode, so `slide-review` does not use it: its `capture.mjs` and the gate scripts launch their own headless Chrome over CDP for the fixed-size screenshots.
