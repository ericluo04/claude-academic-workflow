...
| `/compile-latex` diff-vs-last-compile warning set | [`scunning1975/MixtapeTools:compiledeck`](https://github.com/scunning1975/MixtapeTools) | Scott Cunningham | Persist the warning/box set between compiles and report deltas (`+2 overfull boxes, -1 undefined ref`). The rest of `/compile-latex` (engine/bib auto-detect, ranked file:line error report, package-install fixes, file-stack tracker, auto figure-iterate via `/tikz-iterate`) is original to this repo |
...
| `/audit-reproducibility` 5-phase audit | [`pedrohcgs/claude-code-my-workflow`](https://github.com/pedrohcgs/claude-code-my-workflow) | Pedro H.C. Sant'Anna | 5-phase tolerance-based reproducibility check (extract → match → compare → PASS/FAIL → summarize) and the R / Python / Stata multi-language phase structure. The per-claim PASS/FAIL pattern is conceptually adjacent to Scott Cunningham's per-entry verification in `MixtapeTools/bibcheck` (citations, not numbers) — distinct skill, related discipline. |
...
| `/deslop` AI-tell detection taxonomy | [Wikipedia: Signs of AI writing](https://en.wikipedia.org/wiki/Wikipedia:Signs_of_AI_writing) | Wikipedia contributors | The detection taxonomy in `skills/deslop/references/ai-tells.md` — the "delve list" vocabulary, the stock-phrase set, the negative-parallelism patterns, the formatting/markup tells (title-case headings, bold-colon lists, `oaicite`/`contentReference` artifacts), and the vague-attribution markers. **CC BY-SA** (not MIT). The skill code/workflow is MIT; the derived taxonomy text carries CC BY-SA attribution + share-alike. |
...
Scott's `MixtapeTools` repo is the single largest source of ideas in this workflow. Four full skills (`/referee2`, `/blindspot`, `/bibcheck`, `/tikz-iterate`) are direct descendants of his commands, and two more (`/slide-excellence`, `/create-lecture`) borrow specific lenses from his `beautiful_deck` and `tikz` work. `/compile-latex` borrows his `compiledeck` idea of diffing the warning/box set between successive compiles. His `/referee2` design — reimplementing an analysis in a second language and using bug orthogonality to surface errors — is the strongest single pattern in this repo. His `/blindspot` is the rare review skill that catches what the author has stopped seeing, rather than restating what is already on the page. Thank you, Scott, for shipping these in the open.
...
- [Wikipedia: Signs of AI writing](https://en.wikipedia.org/wiki/Wikipedia:Signs_of_AI_writing) — **CC BY-SA**, not MIT. This is the one exception. The `/deslop` AI-tell *taxonomy text* (`skills/deslop/references/ai-tells.md`) is derived from that essay and therefore carries CC BY-SA's attribution + share-alike obligation: redistributions of that derived taxonomy text must credit Wikipedia and remain share-alike. The rest of the `/deslop` skill — the SKILL.md workflow, the domain-whitelist logic, the scoring/scorecard design, and all skill code — is original to this repo and stays under MIT. In short: the *list of tells* is CC BY-SA-derived; the *machinery that acts on the list* is MIT.
