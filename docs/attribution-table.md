...
| `/draft` voice-shaping pattern | original | this repo | Skill-as-style-mirror pattern: reads `personal_config.user.voice_style_ref` exemplar on every invocation; per-section drafting modes (intro / lit review / method / empirics / discussion / limitations / arbitrary paragraph) with project-aware notation conventions. |
| `/deslop` AI-tell detection taxonomy | [Wikipedia: Signs of AI writing](https://en.wikipedia.org/wiki/Wikipedia:Signs_of_AI_writing) | Wikipedia contributors | The detection taxonomy in `skills/deslop/references/ai-tells.md` — delve-list vocabulary, stock-phrase set, negative-parallelism patterns, formatting/markup tells (title-case headings, bold-colon lists, `oaicite`/`contentReference` artifacts), vague-attribution markers. **CC BY-SA** (not MIT) — attribution + share-alike for the derived taxonomy text. The SKILL.md workflow, domain-whitelist logic, and scorecard design are original to this repo and MIT. |
| `/deslop` skill workflow + domain whitelist | original | this repo | Two-pass mechanical-then-semantic scrub design, the econometrics domain-whitelist + stats-anchor suppression logic, the auto-apply (no-review-diff) output model with a category-count scorecard, and the `--report` / `--mechanical-only` / `--domain` flag surface. The AI-tell *list* it acts on is CC BY-SA-derived (row above); this machinery is MIT. |
| `/referee-response` structure | original | this repo | Sectioned R&R response letter scaffold (Senior Editor / Associate Editor / Reviewer 1 / Reviewer 2), reviewer quotes in `\textit{...}`, location-pinned changes, "Done." for trivials, polite-pushback with cited authority, abandoned drafts kept in `\begin{comment}` blocks. The `--five-q` opt-in hostile-editor stress test is credited separately to aspi6246 (see ATTRIBUTION.md). |
| `/cite` (DOI/arXiv/title → Zotero → .bib append) | original | this repo | Identifier-resolution flow via Semantic Scholar / OpenAlex / arXiv MCPs, with Zotero Web API for adds (MCP is read-only) and Better-BibTeX key generation |
| `/litreview` multi-source dedupe | original | this repo | Cross-source dedupe across arXiv / Semantic Scholar / OpenAlex MCPs (with Crossref fallback) by DOI / arXiv-ID / fuzzy title. The `--four-axis` Novelty/Credibility/Relevance/Actionability mode is credited separately to Blattman's `tips-curate` (see ATTRIBUTION.md). |
| `/bibcheck` per-entry subagent fanout | [scunning1975/MixtapeTools](https://github.com/scunning1975/MixtapeTools) | Scott Cunningham | One-subagent-per-entry pattern to avoid gradient decay |
| `/audit-reproducibility` | [pedrohcgs/claude-code-my-workflow](https://github.com/pedrohcgs/claude-code-my-workflow) | Pedro H.C. Sant'Anna | 5-phase tolerance-based numeric-claim audit and R / Python / Stata multi-language phase structure. The per-claim PASS/FAIL pattern is conceptually adjacent to Scott's per-entry verification in [`MixtapeTools/bibcheck`](https://github.com/scunning1975/MixtapeTools) (citations, not numbers) — distinct skill, related discipline. |
| `/referee2` cross-language replication | [scunning1975/MixtapeTools](https://github.com/scunning1975/MixtapeTools) | Scott Cunningham | Reimplement-in-second-language audit pattern |
...
| `/council` skill | [chrisblattman/claudeblattman](https://github.com/chrisblattman/claudeblattman) | Chris Blattman | Parallel-critics-then-synthesis pattern |
| `/evaluate-idea-marketing` rubric loop | original | this repo | 8-step pre-execution scoring against MKSCI / JMR / JCR / Management Science quality bar, with pivot-loop until score >= 7/10. No public Claude-Code-skill counterpart found. |
| `/evaluate-idea-science` rubric loop | original | this repo | Same 8-step rubric tuned for broad-science venues (PNAS / Nature Human Behaviour / Nature). No public Claude-Code-skill counterpart found. |
...
| `/compile-latex` (shipped) | [`scunning1975/MixtapeTools`](https://github.com/scunning1975/MixtapeTools) (`compiledeck`) + this repo | Scott Cunningham + this repo | Diff-vs-last-compile warning-set idea from Scott's `compiledeck`; everything else original — engine/bib auto-detect, ranked file:line error report with command→package install fixes, file-stack tracker for `\input` attribution, and post-compile auto figure-iterate via `/tikz-iterate` |
| `pre-compact.py` and `post-compact-restore.py` hooks | [`pedrohcgs/claude-code-my-workflow`](https://github.com/pedrohcgs/claude-code-my-workflow) | Pedro H.C. Sant'Anna | Both hooks adapted near-verbatim, including the DRAFT-plan opt-in block (`CLAUDE_PRECOMPACT_BLOCK_ON_DRAFT`), the session-dir hash convention, and the active-plan + recent-decisions snapshot format |
| `format-on-edit.py` hook | original | this repo | Auto-format edited files (ruff for Python, styler for R); silent-on-success, logs failures only. No public counterpart found. |
...
All upstream repos cited above are MIT-licensed at the time of this writing, with one exception: the `/deslop` AI-tell taxonomy is derived from the Wikipedia essay "Signs of AI writing", which is **CC BY-SA**. That derived taxonomy text (`skills/deslop/references/ai-tells.md`) carries CC BY-SA's attribution + share-alike obligation; the surrounding `/deslop` skill code stays MIT. See [ATTRIBUTION.md](../ATTRIBUTION.md) for the full license-compatibility note. Where a skill is a direct port, the original copyright is preserved in the skill's `SKILL.md` header. Where a skill takes only a pattern (a structural idea, a fanout topology) and reimplements from scratch, the row above describes the borrowing and that's the extent of the attribution obligation.
